...
public class login extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String password = "select * from member_penson '"+mpw+"' as pwd";   //사용자가 입력한 패스워드 암호화시킴
			...
			//	HttpSession session = result.getSession();
				//아이디
			//	session.setAttribute("userid", rs.betStrinf)
			...
			String sql_pw = ""; //DB에 저장된 사용자 패스워드
 			while(rs2.next()) { //Database에서 문자형 변수로 변환 작업하는 코드
				sql_id = rs2.getString("mid").intern();
				sql_pw = rs2.getString("mpw").intern();
			}
 			con2.close();
 			ps.close();
